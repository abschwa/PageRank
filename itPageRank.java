...
class Tuple4 implements Comparator<Tuple4>, Comparable<Tuple4>{
    private Integer key;
    private Double pagerank;
    Tuple4(){}
    Tuple4(Integer key, Double pagerank){
        this.key = key;
        this.pagerank = pagerank;
    }

    public Integer getTuple4Key(){
        return this.key;
    }
    public Double getTuple4PageRank(){
        return this.pagerank;
    }

    public int compareTo(Tuple4 t){
        return (this.key).compareTo(t.key);
    }

    public int compare(Tuple4 t1, Tuple4 t2){
        if(t1.getTuple4PageRank() > t2.getTuple4PageRank()) return -1;
        if(t1.getTuple4PageRank() < t2.getTuple4PageRank()) return 1;
        return 0;
    }
...
public class itPageRank {
    ...
    public void printValues() throws IOException {
        ...
        for(int i= 0; i < this.totalURLs; i++){
            list.add(new Tuple4(i, rankValues.get(i)));
        }

        Collections.sort(list, new Tuple4());
    ...
    public static void main(String[] args) throws IOException {
        ...
        itPageRank mpiPR = new itPageRank();
