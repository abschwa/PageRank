...
class Tuple2 implements Comparator<Tuple2>, Comparable<Tuple2>{
    private Integer key;
    private Double pagerank;
    Tuple2(){}
    Tuple2(Integer key, Double pagerank){
        this.key = key;
...
public class itSeqPageRank {
	...
	// this.delta = Double.parseDouble(args[2]);
     ...
     *
     ...
     9 4
     10 4
     * The first value in each line is a URL. Each value after the first value is the URLs referred by the first U$
	...
	for (Integer key : adjMatrix.keySet()){
	    ...
	    */
	    ...
	    //      placeBol = false;
	    //   for(Integer key: rankValuesIntermed.keySet()){
	    //      if (Math.abs(rankValuesIntermed.get(key) - rankValues.get(key)) > this.delta){
	    //          placeBol = true;
	    //      }
    ...
    public void printValues() throws IOException {
        ...
        for(int i= 0; i < this.size; i++){
            list.add(new Tuple2(i, rankValues.get(i)));
        }

        Collections.sort(list, new Tuple2());
    ...
    public static void main(String[] args) throws IOException {
        itSeqPageRank itPR = new itSeqPageRank();

        itPR.parseArgs(args);
        itPR.loadInput();
	itPR.calculatePageRank();
        itPR.printValues();
    }
}
